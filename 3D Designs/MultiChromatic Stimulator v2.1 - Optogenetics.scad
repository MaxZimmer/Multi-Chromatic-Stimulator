...
sep = 1;            // x & y separation values for display function
...
module Labels(){
    translate([-x_Box/2+2,y_Box/2-18,z_Box-Wall/2])linear_extrude(h=Wall/2)text("Optogenetics Stimulator",size = 7.5);
    translate([x_Box/2-17.5,y_Box/2-27.5,z_Box-Wall/2])linear_extrude(h=Wall/2)text("v2.1",size=5.5);
    translate([-x_Box/2+19,y_Box/2-35,z_Box-Wall/2])linear_extrude(h=Wall/2)text("An open hardware LED Stimulator",size=5);
...
module Logo(){
    translate([-x_Box/2+92,-y_Box/2+25,z_Box-Wall/2])scale([21,21,5])scale([0.175,0.175,1])import("C:/Users/Maxime/Documents/GitHub/Multi Chromatic Stimulator/3D Designs/Logo/Trend.stl");
}
...
module LED_neg()translate(pos_LED){
    translate([Wall,0,-x_LED/2])cube([x_LED,2*Wall,x_LED]);
    translate([Wall+x_LED+Wall,0,-x_LED/2])cube([x_LED,2*Wall,x_LED]);
    translate([Wall+2*x_LED+2*Wall,0,-x_LED/2])cube([x_LED,2*Wall,x_LED]);
    translate([Wall+3*x_LED+3*Wall,0,-x_LED/2])cube([x_LED,2*Wall,x_LED]);
    
    translate([Wall/2,0,-x_LED/2-Wall/2])cube([n_LED*x_LED+n_LED*Wall,2*Wall,x_LED+Wall]);
    
    translate([1.75*Wall,2*Wall,-x_LED/2+Wall/2])rotate([90,0,0])linear_extrude(h=Wall/2)text("1",size=7);
    translate([1.75*Wall+x_LED+Wall,2*Wall,-x_LED/2+Wall/2])rotate([90,0,0])linear_extrude(h=Wall/2)text("2",size=7);
    translate([1.75*Wall+2*x_LED+2*Wall,2*Wall,-x_LED/2+Wall/2])rotate([90,0,0])linear_extrude(h=Wall/2)text("3",size=7);
    translate([1.75*Wall+3*x_LED+3*Wall,2*Wall,-x_LED/2+Wall/2])rotate([90,0,0])linear_extrude(h=Wall/2)text("4",size=7);
}
H_LED = 1;
R_LED = 3.9/2;
