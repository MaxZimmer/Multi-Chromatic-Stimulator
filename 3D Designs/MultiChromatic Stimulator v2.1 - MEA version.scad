...
module Labels(){
    translate([-x_Box/2+2,y_Box/2-20,z_Box-Wall/2])linear_extrude(h=Wall/2)text("Multi-Chromatic Stimulator",size = 6.5);
    translate([x_Box/2-32.5,y_Box/2-27.5,z_Box-Wall/2])linear_extrude(h=Wall/2)text("v2.1",size=5);
    translate([-x_Box/2+40,y_Box/2-40,z_Box-Wall/2])linear_extrude(h=Wall/2)text("An open hardware LED Stimulator",size=4);
    ...
    translate([x_Box/2+Wall/2,-12,z_Box/2-3])rotate([90,0,0])rotate([0,90,0])linear_extrude(h=Wall/2)text("Power in",size=5);
    //translate([x_Box/2+Wall/2,-9,z_Box/2-2])rotate([90,0,0])rotate([0,90,0])linear_extrude(h=Wall/2)text("5 - 30V",size=5);
    
    //translate([x_Box/2+Wall/2,-30,z_Box/2+0])rotate([90,0,90])linear_extrude(h=Wall/2)text("LED Channel",size=5);
    translate([x_Box/2+Wall/2,-27,z_Box/2+15])rotate([90,0,90])linear_extrude(h=Wall/2)text("1",size=5);
    translate([x_Box/2+Wall/2,-17,z_Box/2+15])rotate([90,0,90])linear_extrude(h=Wall/2)text("2",size=5);
    translate([x_Box/2+Wall/2,-7,z_Box/2+15])rotate([90,0,90])linear_extrude(h=Wall/2)text("3",size=5);
    translate([x_Box/2+Wall/2,3,z_Box/2+15])rotate([90,0,90])linear_extrude(h=Wall/2)text("4",size=5);
    translate([x_Box/2+Wall/2,13,z_Box/2+15])rotate([90,0,90])linear_extrude(h=Wall/2)text("5",size=5);
    translate([x_Box/2+Wall/2,23,z_Box/2+15])rotate([90,0,90])linear_extrude(h=Wall/2)text("6",size=5);

}

module Logo(){
    translate([-x_Box/2+80,-y_Box/2+5,z_Box-Wall/2])scale([0.85,0.85,1])scale([0.175,0.175,1])import("C:/Users/Maxime/Documents/GitHub/Openspritzer/3D Printing Files/v1.3 - Customed PCB/Badenlab_logo.stl");
}
...
module LED_Holder()translate(pos_LED+[0.9,0,-5.2]){
    difference(){
        union(){
            translate([tol,0,tol])cube([n_LED*x_LED+n_LED*Wall-2*tol,z_Sheet,x_LED+Wall-2*tol]);
        ...
        translate([Wall/2+tol,0,tol+Wall/2])cube([x_LED,z_Sheet+Wall/2+d_LED,x_LED]);
        translate([Wall/2+tol+x_LED+Wall,0,tol+Wall/2])cube([x_LED,z_Sheet+Wall/2+d_LED,x_LED]);
        translate([Wall/2+tol+2*x_LED+2*Wall,0,tol+Wall/2])cube([x_LED,z_Sheet+Wall/2+d_LED,x_LED]);
        translate([Wall/2+tol+3*x_LED+3*Wall,0,tol+Wall/2])cube([x_LED,z_Sheet+Wall/2+d_LED,x_LED]);
...
module Potentiometer()translate([x_Box/2-y_pot,0,pos_z_pot+Wall/2]){
    translate([0,-x_pot/2,0])cube([y_pot,x_pot,z_pot]);
    
    translate([y_pot,d_pot/2,z_pot2])rotate([0,90,0])cylinder(r=r_pot,h=2*Wall,$fn=Smoothness);
    translate([y_pot,-d_pot/2,z_pot2])rotate([0,90,0])cylinder(r=r_pot,h=2*Wall,$fn=Smoothness);
    translate([y_pot,d_pot*3/2,z_pot2])rotate([0,90,0])cylinder(r=r_pot,h=2*Wall,$fn=Smoothness);
    translate([y_pot,-d_pot*3/2,z_pot2])rotate([0,90,0])cylinder(r=r_pot,h=2*Wall,$fn=Smoothness);
    translate([y_pot,d_pot*5/2,z_pot2])rotate([0,90,0])cylinder(r=r_pot,h=2*Wall,$fn=Smoothness);
    translate([y_pot,-d_pot*5/2,z_pot2])rotate([0,90,0])cylinder(r=r_pot,h=2*Wall,$fn=Smoothness);
...
module Potentiometer_Platform(){
   ...
   translate([x_Box/2-y_pot-Wall,-x_pot/2-Wall,pos_z_pot])difference(){
        cube([y_pot+Wall,x_pot+2*Wall,1.5*Wall]);
        
       translate([Wall-tol,Wall-tol,Wall/2])cube([y_pot+tol,x_pot+2*tol,z_pot]);
       translate([2*Wall,2*Wall,-Wall])cube([y_pot,x_pot-2*Wall,z_pot]);
    }

    difference(){
        union(){
            translate([x_Box/2,-x_pot/2+Wall,pos_z_pot-z_Support])rotate(rotate_Support+[0,0,-90])cube([x_Support,z_Support,y_Support]);
            translate([x_Box/2,x_pot/2+Wall,pos_z_pot-z_Support])rotate(rotate_Support+[0,0,-90])cube([x_Support,z_Support,y_Support]);
        }
        translate([x_Box/2-y_pot-Wall,-x_pot/2-Wall,pos_z_pot+Wall/2])cube([y_pot+Wall,x_pot+2*Wall,z_Support]);
        translate([x_Box/2,-x_pot/2-Wall,pos_z_pot+Wall/2-z_Support])cube([y_pot+Wall,x_pot+2*Wall,z_Support]);
        translate([x_Box/2-y_pot-Wall,-x_pot/2-Wall,pos_z_pot+Wall/2])translate([Wall*3/2,Wall*3/2,-tol])cube([y_pot-Wall,x_pot-Wall,z_pot]);
    }
}
